...
Decision path: <code>{validation.get('decision_path', 'not recorded')}</code>; gate action: <code>{(validation.get('gate_decision') or {}).get('action', 'not recorded')}</code>; gate reason code: <code>{(validation.get('gate_decision') or {}).get('reason_code', 'not recorded')}</code>; gate reason: <code>{(validation.get('gate_decision') or {}).get('reason_text', 'not recorded')}</code>; soft artifact status: <code>{soft_challenge.get('status', 'not recorded')}</code>; calibration metadata is non-authoritative in production; reconciliation invoked: <code>{soft_challenge.get('reconciliation_invoked', False)}</code>.

### Limited training-only empirical comparison

Probe invoked: <code>{validation.get('empirical_probe_invoked', False)}</code>; status: <code>{validation.get('probe_status', empirical_probe.get('status', 'not invoked'))}</code>; policy: <code>{empirical_probe.get('policy_version', 'not invoked')}</code>; metric: <code>{empirical_probe.get('metric', 'not invoked')}</code>; folds: <code>{empirical_probe.get('cv_folds', 'not invoked')}</code>; winner: <code>{empirical_probe.get('winner', 'not invoked')}</code>; evidence strength: <code>{validation.get('probe_evidence_strength', empirical_probe.get('evidence_strength', 'not invoked'))}</code>; probe/gate reason code: <code>{(validation.get('gate_decision') or {}).get('reason_code', validation.get('abstention_reason', 'none'))}</code>.
...
Holdout boundary: the formulation gate completed and was validated before the supervised split. Modeling-agent evidence, deterministic structural-challenger evidence, modeling reconciliation, preprocessing requirements, structural-cleaning decisions, pre-evaluation EDA and plots, and cross-validation used training-partition evidence only. The LLM receives a compact profile while the challenger may use richer bounded structural diagnostics from the same training partition. The EDA artifact contains <code>{eda.get('rows', 0)}</code> cleaned training rows and no holdout rows. Hard validation at the modeling gate is also run on the training partition and carries the frozen split contract as metadata; split establishment may inspect target labels before planning, but holdout outcomes never enter planning or selection. The frozen holdout was scored once for final model evaluation.
