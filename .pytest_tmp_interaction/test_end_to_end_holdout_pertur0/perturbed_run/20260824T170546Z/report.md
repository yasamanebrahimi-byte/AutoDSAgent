...
Deterministic reasoning: The training-only deterministic policy ranked tree_ensemble highest with compatibility score 73 (low confidence). It considered 3 usable features across 80 training rows, estimated 7 post-one-hot features, and observed nominal class association measured with classification_eta_squared_and_cramers_v; maximum class-separation strength was 1.00. The structural-complexity signal was low (0.25). Key positive factors: numeric and categorical predictors are mixed; multiple categorical predictors suggest structured splits; ratio 11.43 supports ensemble fitting. Compatibility scores are policy rankings, not probabilities.
...
- <code>regularized_linear</code>: <code>70.0</code>
- <code>tree_ensemble</code>: <code>73.0</code>
- <code>boosted_tree</code>: <code>67.0</code>

Training-only diagnostics: <code>{"association_measure": "classification_eta_squared_and_cramers_v", "binary_feature_count": 2, "boosted_effective_features_estimate": 3, "categorical_feature_count": 2, "class_separation_strength": 1.0, "effective_features_estimate": 7, "estimated_one_hot_dimensionality": 7, "excluded_feature_types": {"categorical": 1, "identifier": 1, "unknown": 1}, "excluded_features": 3, "features_with_missing_count": 1, "features_with_missing_fraction": 0.16666666666666666, "high_cardinality_feature_count": 0, "high_cardinality_feature_fraction": 0.0, "high_correlation_pair_count": 0, "high_correlation_pair_fraction": 0.0, "interaction_signals": {"candidate_feature_count": 0, "candidate_features": [], "interaction_applicable": false, "interaction_pairs_evaluated": 0, "interaction_score": 0.0, "interaction_strength": "low", "skipped_pair_count": 0, "skipped_pair_reasons": {}, "strong_interaction_pair_count": 0, "strong_interaction_pair_fraction": 0.0, "top_interaction_pairs": []}, "linear_effective_features_estimate": 7, "marginal_association_strength": 0.380056258760433, "max_abs_numeric_correlation": 0.0, "max_categorical_cardinality": 80, "max_feature_missing_fraction": 1.0, "mean_categorical_cardinality": 33.0, "mean_univariate_signal": 0.380056258760433, "missingness_pattern": "concentrated", "nonlinear_feature_count": 0, "nonlinear_feature_fraction": 0.0, "nonlinearity_applicable": false, "nonlinearity_heterogeneity": 0.0, "nonlinearity_score": 0.0, "nonlinearity_signal": "low", "numeric_feature_count": 1, "numeric_outlier_cell_fraction": 0.0, "numeric_outlier_feature_fraction": 0.0, "overall_missing_fraction": 0.16666666666666666, "pearson_spearman_gap": 0.0, "rows": 80, "sample_to_feature_ratio": 11.428571428571429, "structural_complexity_score": 0.25, "structural_complexity_signal": "low", "target": {"classification": {"classes": 2, "imbalance_ratio": 1.0, "majority_class_fraction": 0.5, "minimum_class_size": 40, "minority_class_fraction": 0.5, "samples_per_class": {"class_1": 40, "class_2": 40}}, "regression": null}, "text_feature_count": 0, "training_row_count": 80, "tree_effective_features_estimate": 7, "usable_features": 3}</code>.
...
- <code>feature_composition</code> (+7): numeric and categorical predictors are mixed
- <code>categorical_structure</code> (+5): multiple categorical predictors suggest structured splits
- <code>categorical_cardinality</code> (-1): maximum categorical cardinality 80 burdens one-hot trees
- <code>sample_to_feature_ratio</code> (+6): ratio 11.43 supports ensemble fitting
- <code>dataset_scale</code> (+3): 80 rows support a modest ensemble
- <code>encoded_dimensionality</code> (+2): estimated encoded dimension 7 is low
- <code>missingness</code> (+1): overall missing-cell fraction 0.17; imputation remains available
...
Holdout boundary: the formulation gate completed and was validated before the supervised split. Modeling-agent evidence, deterministic recommendation evidence, modeling reconciliation, preprocessing requirements, structural-cleaning decisions, pre-evaluation EDA and plots, and cross-validation used training-partition evidence only. The EDA artifact contains <code>80</code> cleaned training rows and no holdout rows. The fail-closed validation gate may inspect the full raw or cleaned frame only to enforce target, schema, feasibility, and frozen-membership invariants; those guardrail checks are not planning evidence. The frozen holdout was scored once for final model evaluation.
...
Excluded features and reasons: <code>numeric_text (identifier_like)</code>.
...
- Columns before cleaning: <code>7</code>
- Duplicate rows detected: <code>0</code>
- Rows after cleaning: <code>100</code>
- Columns after cleaning: <code>5</code>
...
- The cleaned analysis frame contains 80 rows and 5 columns.
- No missing values were present in the cleaned frame.
- The strongest absolute numeric relationship observed was signal with numeric_text (absolute r=1.000).
