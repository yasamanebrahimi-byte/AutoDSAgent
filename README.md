...
These figures were generated by a real end-to-end run using the 569-row Breast Cancer Wisconsin (Diagnostic) public benchmark and target `diagnosis`.

Reference run: 455 training rows, 114 held-out rows, 5-fold training-only CV.
Logistic regression achieved CV macro F1 `0.9716 ± 0.0162` and holdout macro
F1 `0.9619` (accuracy `0.9649`). These are deterministic demo results, not a
clinical claim.

### Inspect target balance

![Diagnostic target class distribution generated by AutoDS Agent](docs/screenshots/target-distribution.png)

### Surface numeric relationships

![Numeric correlation heatmap generated by AutoDS Agent](docs/screenshots/correlation-heatmap.png)

### Compare cross-validated models

![Cross-validated model comparison generated by AutoDS Agent](docs/screenshots/model-comparison.png)
...
Bundled, reproducible public benchmarks live in `examples/sample_data/`.

| Dataset                              | Rows | File                          | Target                | Task           |
| ------------------------------------ | ---: | ----------------------------- | --------------------- | -------------- |
| Breast Cancer Wisconsin (Diagnostic) |  569 | `breast_cancer_wisconsin.csv` | `diagnosis`           | Classification |
| Diabetes disease progression         |  442 | `diabetes_progression.csv`    | `disease_progression` | Regression     |

The benchmark CSVs are exported from datasets distributed with scikit-learn by
`python scripts/export_benchmark_datasets.py`. Their source links, attribution,
licenses, row counts, and SHA-256 hashes are recorded in
`examples/sample_data/benchmark_manifest.json`. The datasets demonstrate the
software workflow only and must not be used for clinical decisions.
...
- [Verified Demo Gallery](docs/screenshots/README.md)
