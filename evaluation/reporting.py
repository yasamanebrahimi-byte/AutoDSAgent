...
def render_summary_markdown(
    ...
    rows = [
        ...
        f"| Live required | {summary.get('live_required', config.get('require_live', False))} |",
        ...
        f"- Challenges: **{summary.get('total_challenges', summary.get('challenges', 0))} / {summary.get('total_disagreements', summary.get('soft_challenge_count', 0))} actionable model-family disagreements**; abstentions: **{summary.get('total_abstentions', summary.get('abstentions', 0))}**.",
        f"- Holdout intervention outcomes: beneficial **{summary.get('improved_interventions', summary.get('challenge_improved_count', 0))}**; harmful **{summary.get('worsened_interventions', summary.get('challenge_worsened_count', 0))}**; neutral **{summary.get('neutral_interventions', summary.get('challenge_neutral_count', 0))}**.",
        f"- Training-reference diagnostics: challenge yield **{_percent(summary.get('training_reference_challenge_yield', summary.get('challenge_yield')))}**; harmful-reference rate **{_percent(summary.get('training_reference_harmful_intervention_rate'))}**; unnecessary-reference rate **{_percent(summary.get('training_reference_unnecessary_intervention_rate', summary.get('unnecessary_intervention_rate')))}**.",
        f"- Paper-facing rates: challenge **{_percent(summary.get('challenge_rate'))}**, intervention **{_percent(summary.get('intervention_rate'))}**, abstention/preservation **{_percent(summary.get('abstention_preservation_rate'))}**, beneficial **{_percent(summary.get('beneficial_intervention_rate'))}**, harmful **{_percent(summary.get('harmful_intervention_rate'))}**, neutral **{_percent(summary.get('neutral_intervention_rate'))}**.",
        ...
        f"- Within-model-condition paper holdout delta (dataset-macro mean): **{_number(summary.get('dataset_macro_paper_holdout_delta_mean'))}**; median: **{_number(summary.get('dataset_macro_paper_holdout_delta_median'))}**; clustered CI: `{summary.get('dataset_macro_paper_holdout_delta_ci')}`. Classification and regression magnitudes are reported separately below because their units differ.",
        "",
        "| Metric | Dataset-macro (primary) | Trial-weighted (secondary diagnostic) |",
        "|---|---:|---:|",
        f"| Intervention precision | {_percent(summary.get('dataset_macro_gate_health', {}).get('intervention_precision', summary.get('intervention_precision')))} | {_percent(summary.get('trial_weighted_gate_health', {}).get('intervention_precision'))} |",
        f"| Holdout harmful-intervention rate | {_percent(summary.get('dataset_macro_gate_health', {}).get('harmful_intervention_rate', summary.get('harmful_intervention_rate')))} | {_percent(summary.get('trial_weighted_gate_health', {}).get('harmful_intervention_rate'))} |",
        f"| Mean regret reduction (secondary diagnostic) | {_number(summary.get('dataset_macro_gate_health', {}).get('mean_regret_reduction', summary.get('mean_regret_reduction')))} | {_number(summary.get('trial_weighted_gate_health', {}).get('mean_regret_reduction'))} |",
f"| Within-model-condition paper holdout delta | {_number(summary.get('dataset_macro_paper_holdout_delta_mean'))} | {_number(summary.get('paper_holdout_delta_mean'))} |",
        "",
    ...
    rows.extend([
        ...
        f"- Training-reference challenge outcomes: **{summary.get('challenge_improved_count', 0)} improved**, **{summary.get('challenge_worsened_count', 0)} worsened**, **{summary.get('challenge_neutral_count', 0)} neutral**; holdout intervention precision: **{_percent(summary.get('intervention_precision'))}**.",
        ...
        f"- Mean deterministic-challenger regret advantage: **{_number(summary.get('mean_performance_delta_conditional_on_challenge'))}**; this is `agent normalized regret - deterministic challenger normalized regret`, so it is not final gated intervention improvement; training-reference unnecessary interventions: **{summary.get('unnecessary_intervention_count', 0)}** (**{_percent(summary.get('training_reference_unnecessary_intervention_rate', summary.get('unnecessary_intervention_rate')))}**).",
        ...
        f"- OpenAI-only mean paired paper holdout delta: **{_number(openai_pair.get('mean_paired_holdout_improvement'))}** (dimensionless; subgroup descriptive only).",
        ...
        f"- Classification dataset-macro macro-F1 change: **{_number(summary.get('paper_metrics_by_task', {}).get('classification', {}).get('dataset_macro', {}).get('mean_paper_holdout_delta'))}**; clustered CI: `{summary.get('paper_metrics_by_task', {}).get('classification', {}).get('dataset_macro_confidence_intervals', {}).get('mean_paper_holdout_delta')}`.",
        f"- Regression dataset-macro relative RMSE improvement: **{_number(summary.get('paper_metrics_by_task', {}).get('regression', {}).get('dataset_macro', {}).get('mean_paper_holdout_delta'))}**; clustered CI: `{summary.get('paper_metrics_by_task', {}).get('regression', {}).get('dataset_macro_confidence_intervals', {}).get('mean_paper_holdout_delta')}`.",
        "",
    ...
    rows.extend([
        ...
        "",
        f"- Initial hard-invalid proposals: **{summary.get('initial_hard_invalid_proposal_count', summary.get('agent_initial_invalid_count', 0))}**; hard interceptions: **{summary.get('hard_interception_count', summary.get('unsafe_plan_interception_count', 0))}**; hard repairs: **{summary.get('hard_repair_count', 0)}**; soft interventions: **{summary.get('soft_intervention_count', 0)}**.",
