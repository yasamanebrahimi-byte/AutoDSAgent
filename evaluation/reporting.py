...
def render_summary_markdown(
    ...
    rows = [
        ...
        "## Gate Health",
        "",
        "Intervention quality is primary. The headline estimate is dataset-macro: each eligible `benchmark_case` contributes one equally weighted dataset/task summary. Trial-weighted values below are secondary diagnostics. Exact family match and top-2 compatibility are also secondary diagnostics.",
        ...
        f"| Mean regret reduction | {_number(summary.get('dataset_macro_gate_health', {}).get('mean_regret_reduction', summary.get('mean_regret_reduction')))} | {_number(summary.get('trial_weighted_gate_health', {}).get('mean_regret_reduction'))} |",
        "",
